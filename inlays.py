...
def svg_inlay_workflow(
    svg_path=None,
    min_bit_radius_inch=None,
    pad_depth_inch=None,
    final_solid_name=None,
    tiny_feature_policy=None,
    attempt_tiny_feature_fix=None,
    show_issue_dialog=None,
    show_dialog=True,
):
    print("SVG Inlay Workflow has been removed from this workbench.")
    return

    workflow_prefs = None
    default_svg_path = ""
    default_min_bit_radius_inch = 0.014
    default_pad_depth_inch = 0.2
    default_final_name = "final_inlay"
    default_tiny_feature_policy = "abort"
    default_attempt_tiny_feature_fix = True
    default_show_issue_dialog = True
    try:
        workflow_prefs = App.ParamGet("User parameter:BaseApp/Preferences/Mod/ButlerCues/SVGInlayWorkflow")
        default_svg_path = str(workflow_prefs.GetString("svg_path", "") or "").strip()
        default_min_bit_radius_inch = max(0.0001, float(workflow_prefs.GetFloat("min_bit_radius_in", 0.014)))
        default_pad_depth_inch = max(0.0001, float(workflow_prefs.GetFloat("pad_depth_in", 0.2)))
        default_final_name = str(workflow_prefs.GetString("final_name", "final_inlay") or "").strip() or "final_inlay"
        default_tiny_feature_policy = str(workflow_prefs.GetString("tiny_feature_policy", "abort") or "abort").strip().lower()
        default_attempt_tiny_feature_fix = bool(workflow_prefs.GetBool("attempt_tiny_feature_fix", True))
        default_show_issue_dialog = bool(workflow_prefs.GetBool("show_issue_dialog", True))
    except Exception:
        workflow_prefs = None

    if svg_path is None:
        svg_path = default_svg_path
    svg_path = str(svg_path or "").strip()

    if min_bit_radius_inch is None:
        min_bit_radius_inch = default_min_bit_radius_inch
    try:
        min_bit_radius_inch = max(0.0001, float(min_bit_radius_inch))
    except Exception:
        min_bit_radius_inch = default_min_bit_radius_inch

    if pad_depth_inch is None:
        pad_depth_inch = default_pad_depth_inch
    try:
        pad_depth_inch = max(0.0001, float(pad_depth_inch))
    except Exception:
        pad_depth_inch = default_pad_depth_inch

    output_label = str(final_solid_name or default_final_name or "final_inlay").strip() or "final_inlay"

    if tiny_feature_policy is None:
        tiny_feature_policy = default_tiny_feature_policy
    tiny_feature_policy = str(tiny_feature_policy or "abort").strip().lower()
    if tiny_feature_policy not in {"abort", "warn", "skip"}:
        tiny_feature_policy = "abort"

    if attempt_tiny_feature_fix is None:
        attempt_tiny_feature_fix = default_attempt_tiny_feature_fix
    try:
        attempt_tiny_feature_fix = bool(attempt_tiny_feature_fix)
    except Exception:
        attempt_tiny_feature_fix = True

    if show_issue_dialog is None:
        show_issue_dialog = default_show_issue_dialog
    try:
        show_issue_dialog = bool(show_issue_dialog)
    except Exception:
        show_issue_dialog = True

    if show_dialog and QtGui is not None and Gui is not None:
        class _SVGInlayWorkflowTaskPanel:
            def __init__(self):
                self.form = QtGui.QWidget()
                self.form.setWindowTitle("SVG to Inlay Workflow")
                self._last_run_signature = None

                layout = QtGui.QFormLayout(self.form)

                path_row = QtGui.QWidget()
                path_layout = QtGui.QHBoxLayout(path_row)
                path_layout.setContentsMargins(0, 0, 0, 0)
                self.path_edit = QtGui.QLineEdit(str(svg_path or ""))
                ...
                self.radius_spin = QtGui.QDoubleSpinBox()
                self.radius_spin.setDecimals(4)
                self.radius_spin.setRange(0.0001, 1.0000)
                self.radius_spin.setSingleStep(0.001)
                self.radius_spin.setValue(float(min_bit_radius_inch))
                self.radius_spin.setSuffix(" in")

                self.pad_spin = QtGui.QDoubleSpinBox()
                self.pad_spin.setDecimals(4)
                self.pad_spin.setRange(0.0001, 5.0000)
                self.pad_spin.setSingleStep(0.01)
                self.pad_spin.setValue(float(pad_depth_inch))
                self.pad_spin.setSuffix(" in")

                self.final_name_edit = QtGui.QLineEdit(str(output_label or "final_inlay"))

                self.auto_fix_tiny_check = QtGui.QCheckBox("Use CAM-style auto-fix for bit-limited corners")
                self.auto_fix_tiny_check.setChecked(bool(attempt_tiny_feature_fix))

                note = QtGui.QLabel(
                    "Imports SVG, applies CAM-style bit-limit cleanup, pads reverse, then fillets Z edges at tool radius."
                )

                layout.addRow("SVG file", path_row)
                layout.addRow("Smallest bit radius", self.radius_spin)
                layout.addRow("Pad depth (reverse)", self.pad_spin)
                layout.addRow("Final solid name", self.final_name_edit)
                layout.addRow("Corner cleanup", self.auto_fix_tiny_check)
                layout.addRow("", note)

                try:
                    self.path_edit.textChanged.connect(self._mark_dirty)
                    self.radius_spin.valueChanged.connect(self._mark_dirty)
                    self.pad_spin.valueChanged.connect(self._mark_dirty)
                    self.final_name_edit.textChanged.connect(self._mark_dirty)
                    self.auto_fix_tiny_check.toggled.connect(self._mark_dirty)
                    self.path_button.clicked.connect(self._browse_svg)
                except Exception:
                    pass
            ...
            def _signature(self):
                return (
                    str(self.path_edit.text() or "").strip(),
                    round(float(self.radius_spin.value()), 6),
                    round(float(self.pad_spin.value()), 6),
                    str(self.final_name_edit.text() or "").strip().lower(),
                    bool(self.auto_fix_tiny_check.isChecked()),
                )

            def _run_workflow(self, close_after=True):
                current_signature = self._signature()
                if close_after and self._last_run_signature == current_signature:
                    try:
                        Gui.Control.closeDialog()
                ...
                selected_path = str(self.path_edit.text() or "").strip()
                selected_radius = max(0.0001, float(self.radius_spin.value()))
                selected_pad = max(0.0001, float(self.pad_spin.value()))
                selected_name = str(self.final_name_edit.text() or "").strip() or "final_inlay"
                selected_auto_fix_tiny = bool(self.auto_fix_tiny_check.isChecked())

                try:
                    if workflow_prefs is not None:
                        workflow_prefs.SetString("svg_path", str(selected_path))
                        workflow_prefs.SetFloat("min_bit_radius_in", float(selected_radius))
                        workflow_prefs.SetFloat("pad_depth_in", float(selected_pad))
                        workflow_prefs.SetString("final_name", str(selected_name))
                        workflow_prefs.SetBool("attempt_tiny_feature_fix", bool(selected_auto_fix_tiny))
                except Exception:
                    pass

                try:
                    svg_inlay_workflow(
                        svg_path=selected_path,
                        min_bit_radius_inch=selected_radius,
                        pad_depth_inch=selected_pad,
                        final_solid_name=selected_name,
                        tiny_feature_policy="warn",
                        attempt_tiny_feature_fix=selected_auto_fix_tiny,
                        show_issue_dialog=False,
                        show_dialog=False,
                    )
                    self._last_run_signature = current_signature
                finally:
            ...
            def clicked(self, button):
                if button == QtGui.QDialogButtonBox.Apply:
                    self._run_workflow(close_after=False)
                    return True
                return False

            def accept(self):
                return self._run_workflow(close_after=True)
        ...
        Gui.Control.showDialog(_SVGInlayWorkflowTaskPanel())
        return

    if not Gui:
        print("SVG inlay workflow requires GUI selection support.")
        return
    ...
    doc = App.ActiveDocument
    if not doc:
        doc = App.newDocument("SVG_Inlay")

    try:
        import importSVG
    except Exception as exc:
        print(f"SVG importer unavailable: {exc}")
        return
    ...
    def _safe_name(text, fallback="svg_inlay"):
        raw = str(text or "").strip()
        cleaned = "".join(ch if ch.isalnum() else "_" for ch in raw).strip("_")
        return cleaned or fallback

    def _is_edge_parallel(edge_obj, axis_vec, tol=1e-6):
        try:
            curve = getattr(edge_obj, "Curve", None)
            if curve and getattr(curve, "TypeId", "") == "Part::GeomLine":
                return bool(curve.Direction.isParallel(axis_vec, tol))
        except Exception:
            pass
        try:
            vertices = list(getattr(edge_obj, "Vertexes", []) or [])
            if len(vertices) != 2:
                return False
            p0 = getattr(vertices[0], "Point", None)
            p1 = getattr(vertices[1], "Point", None)
            ...
            if float(getattr(delta, "Length", 0.0) or 0.0) <= tol:
                return False
            return bool(delta.isParallel(axis_vec, max(1e-4, tol * 10.0)))
        except Exception:
            return False

    def _cleanup_micro_xy_steps(shape_obj, max_step_mm):
        if not shape_obj:
            return shape_obj, 0
        try:
            if shape_obj.isNull():
        ...
        threshold = max(0.01, float(max_step_mm or 0.0))
        if threshold <= 0.01:
            return shape_obj, 0
        ...
        for _ in range(4):
            round_updates = 0
            try:
                edges_now = list(getattr(working, "Edges", []) or [])
            except Exception:
                edges_now = []
            ...
            for edge in edges_now:
                try:
                    ...
                    edge_len = 0.0
                if edge_len <= 0.0 or edge_len > threshold:
                    continue
                if _is_edge_parallel(edge, z_axis):
                    continue
                tiny_xy_edges.append(edge)

            if not tiny_xy_edges:
                break

            for edge in tiny_xy_edges:
                try:
                    edge_len = float(getattr(edge, "Length", 0.0) or 0.0)
                except Exception:
                    continue
                radius_mm = max(0.005, min(edge_len * 0.45, threshold * 0.45))
                try:
                    candidate = working.makeFillet(radius_mm, [edge])
                except Exception:
                    continue
        ...
        try:
            working = working.removeSplitter()
        except Exception:
            pass
        return working, updates

    def _corner_interference_metrics(shape_obj, tiny_len_threshold_mm, min_turn_deg=8.0):
        try:
            wires = list(getattr(shape_obj, "Wires", []) or [])
        except Exception:
            wires = []
        if not wires:
            return []

        limit = max(0.01, float(tiny_len_threshold_mm or 0.0))
        turn_threshold = max(1.0, float(min_turn_deg or 0.0))
        z_axis = App.Vector(0, 0, 1)
        issues = []

        def _edge_vector(edge_obj, at_start):
            try:
                verts = list(getattr(edge_obj, "Vertexes", []) or [])
                if len(verts) != 2:
                    return None
                p0 = getattr(verts[0], "Point", None)
                p1 = getattr(verts[1], "Point", None)
                if p0 is None or p1 is None:
                    return None
                vec = p1.sub(p0) if at_start else p0.sub(p1)
                if float(getattr(vec, "Length", 0.0) or 0.0) <= 1e-9:
                    return None
                return vec
            except Exception:
                return None

        def _turn_angle_deg(v0, v1):
            try:
                l0 = float(getattr(v0, "Length", 0.0) or 0.0)
                l1 = float(getattr(v1, "Length", 0.0) or 0.0)
                if l0 <= 1e-9 or l1 <= 1e-9:
                    return 0.0
                angle_rad = float(v0.getAngle(v1) or 0.0)
        ...
        for wire in wires:
            ...
            edge_count = len(wire_edges)
            for edge_index, edge in enumerate(wire_edges):
                try:
                    seg_len = float(getattr(edge, "Length", 0.0) or 0.0)
                except Exception:
                    seg_len = 0.0
                if seg_len <= 0.0 or seg_len >= limit:
                    continue
                if _is_edge_parallel(edge, z_axis):
                    continue

                prev_edge = wire_edges[(edge_index - 1) % edge_count] if edge_count > 1 else None
                next_edge = wire_edges[(edge_index + 1) % edge_count] if edge_count > 1 else None
                if not prev_edge or not next_edge:
                    continue

                in_vec = _edge_vector(prev_edge, at_start=False)
                out_vec = _edge_vector(next_edge, at_start=True)
                if in_vec is None or out_vec is None:
                    continue

                turn_deg = _turn_angle_deg(in_vec, out_vec)
                if turn_deg >= turn_threshold:
                    issues.append((seg_len, turn_deg))

        return issues

    def _remove_corner_interference_steps(shape_obj, tiny_len_threshold_mm, min_turn_deg=8.0):
        try:
            wires = list(getattr(shape_obj, "Wires", []) or [])
        except Exception:
            wires = []
        ...
        total_removed = 0
        working_edges = list(wire_edges)

        for _ in range(4):
            if len(working_edges) < 3:
                break

            remove_indices = set()
            edge_count = len(working_edges)
            for edge_index, edge in enumerate(working_edges):
                try:
                    seg_len = float(getattr(edge, "Length", 0.0) or 0.0)
                except Exception:
                    seg_len = 0.0
                ...
                prev_edge = working_edges[(edge_index - 1) % edge_count]
                next_edge = working_edges[(edge_index + 1) % edge_count]
            ...
            kept_edges = [edge for idx, edge in enumerate(working_edges) if idx not in remove_indices]
            if len(kept_edges) < 2:
                break

            try:
                grouped = Part.sortEdges(kept_edges)
            except Exception:
                grouped = [kept_edges]
            ...
            try:
                next_wire = Part.Wire(grouped[0])
            except Exception:
                break

            try:
                if wire.isClosed() and (not next_wire.isClosed()):
                    break
            except Exception:
                pass

            try:
                working_edges = list(getattr(next_wire, "Edges", []) or [])
            except Exception:
                break
        ...
        try:
            grouped_final = Part.sortEdges(working_edges)
        except Exception:
            grouped_final = [working_edges]
        if not grouped_final or len(grouped_final) != 1:
            return shape_obj, 0

        try:
            final_wire = Part.Wire(grouped_final[0])
        except Exception:
            return shape_obj, 0

        try:
            if wire.isClosed() and (not final_wire.isClosed()):
                return shape_obj, 0
        except Exception:
            pass

        try:
            new_face = Part.Face(final_wire)
            if not new_face or new_face.isNull() or (not new_face.isValid()):
                return shape_obj, 0
            return new_face, total_removed
        except Exception:
            return shape_obj, 0

    def _highlight_offending_solids(offending):
        for _, offending_obj, _, _ in list(offending or []):
            try:
                view_obj = getattr(offending_obj, "ViewObject", None)
                if not view_obj:
                    continue
                view_obj.ShapeColor = (1.0, 0.35, 0.35)
                view_obj.LineColor = (1.0, 0.1, 0.1)
                view_obj.LineWidth = 3.0
            except Exception:
                continue

    def _show_tiny_feature_report(offending, bit_dia_mm, policy_mode):
        if not offending or not QtGui or not Gui or not show_issue_dialog:
            return
        try:
            lines = []
            for label, _, tiny_count, tiny_min in list(offending)[:100]:
                lines.append(
                    f"{label}: {int(tiny_count)} fillet-interfering corner step(s), smallest={float(tiny_min) / 25.4:.4f} in"
                )

            mode_text = {
                "abort": "Current policy: Abort and highlight offending solids.",
                "warn": "Current policy: Warn and continue.",
                "skip": "Current policy: Skip offending solids.",
            }.get(str(policy_mode or ""), "Current policy: Abort and highlight offending solids.")

            msg = QtGui.QMessageBox()
            msg.setIcon(QtGui.QMessageBox.Warning)
            msg.setWindowTitle("SVG Workflow Machinability Issues")
            msg.setText(
                f"Found {len(offending)} shape(s) with corner-step interference for bit diameter "
                f"({float(bit_dia_mm) / 25.4:.4f} in)."
            )
            msg.setInformativeText(
                "Checks focus on tiny segments that introduce real corner turns and commonly break filleting. "
                "Offending solids are highlighted in red when available. " + mode_text
            )
            msg.setDetailedText("\n".join(lines))
            msg.exec_()
        except Exception:
    ...
    def _export_issue_wires(prepared_face_entries, offending, parent_group=None):
        if not prepared_face_entries:
            return None

        issue_labels = {str(label) for label, _, _, _ in list(offending or [])}

        def _add_edge_as_geometry(sketch_obj, edge_obj):
            try:
                curve = getattr(edge_obj, "Curve", None)
                curve_type = str(getattr(curve, "TypeId", "") or "")
            except Exception:
            ...
            if curve_type == "Part::GeomLine" and len(vertices) >= 2:
                try:
                    p0 = vertices[0].Point
                    p1 = vertices[-1].Point
                    if p0.distanceToPoint(p1) > 1e-7:
                        sketch_obj.addGeometry(Part.LineSegment(p0, p1), False)
                        return 1
                except Exception:
                    pass
            ...
            if curve_type == "Part::GeomCircle":
                try:
                    first = float(getattr(edge_obj, "FirstParameter", 0.0) or 0.0)
                    last = float(getattr(edge_obj, "LastParameter", 0.0) or 0.0)
                    span = abs(last - first)
                    if span >= (2.0 * math.pi - 1e-6):
                        sketch_obj.addGeometry(Part.Circle(curve.Center, curve.Axis, curve.Radius), False)
                    else:
                        sketch_obj.addGeometry(Part.ArcOfCircle(curve, first, last), False)
                    return 1
                except Exception:
                    pass

            # Prefer native spline geometry to avoid huge polyline expansions.
            if curve_type in ("Part::GeomBSplineCurve", "Part::GeomBezierCurve") and curve is not None:
                try:
                    spline_curve = curve.copy()
                    try:
                        first = float(getattr(edge_obj, "FirstParameter", 0.0) or 0.0)
                        last = float(getattr(edge_obj, "LastParameter", 0.0) or 0.0)
                        if abs(last - first) > 1e-12 and hasattr(spline_curve, "segment"):
                            spline_curve.segment(first, last)
                    except Exception:
                        pass
                    sketch_obj.addGeometry(spline_curve, False)
                    return 1
                except Exception:
                    pass

            # Fallback: coarse discretize and add polyline segments (capped).
            try:
                pts = list(edge_obj.discretize(Number=16) or [])
            except Exception:
                pts = []
            if len(pts) < 2 and len(vertices) >= 2:
                try:
                    pts = [vertices[0].Point, vertices[-1].Point]
                except Exception:
                    pts = []

            # Hard cap points to avoid million-segment sketches.
            if len(pts) > 64:
                step = max(1, int(math.ceil(float(len(pts)) / 64.0)))
                reduced = [pts[i] for i in range(0, len(pts), step)]
                if reduced[-1].distanceToPoint(pts[-1]) > 1e-7:
        ...
        try:
            issue_group_name = _next_name(_safe_name("SVGIssues", "SVGIssues"))
            issue_group = doc.addObject("App::DocumentObjectGroup", issue_group_name)
            issue_group.Label = "SVG Machinability Issues"
            if parent_group:
                try:
                    parent_group.addObject(issue_group)
                except Exception:
                    pass
        except Exception:
            issue_group = None
        ...
        try:
            machinable_group_name = _next_name(_safe_name("SVGMachinable", "SVGMachinable"))
            machinable_group = doc.addObject("App::DocumentObjectGroup", machinable_group_name)
            machinable_group.Label = "SVG Machinable"
            if parent_group:
                try:
                    parent_group.addObject(machinable_group)
                except Exception:
                    pass
        except Exception:
            machinable_group = None

        created_issue = 0
        created_machinable = 0
        for idx, face_obj in list(prepared_face_entries or []):
            label = f"s{idx}"
            is_issue = label in issue_labels

            exported_wires = []
            try:
                exported_wires = list(getattr(face_obj, "Wires", []) or [])
            except Exception:
                exported_wires = []

            if not exported_wires:
                try:
                    outer_wire = getattr(face_obj, "OuterWire", None)
                    if outer_wire and (not outer_wire.isNull()):
                        exported_wires = [outer_wire]
                except Exception:
                    exported_wires = []
            ...
            sketch_name = _next_name(_safe_name(f"{label}_sketch", "issue_sketch"))
            try:
                sketch_obj = doc.addObject("Sketcher::SketchObject", sketch_name)
                sketch_obj.Label = f"{label}_sketch"
            except Exception:
                sketch_obj = None
            ...
            added_geometries = 0
            for wire_shape in exported_wires:
                try:
                    if not wire_shape or wire_shape.isNull():
                        continue
                except Exception:
                    continue
                ...
                for edge_obj in edges:
                    try:
                        added_geometries += int(_add_edge_as_geometry(sketch_obj, edge_obj) or 0)
                        if added_geometries > 5000:
                            break
                    except Exception:
                        continue
                if added_geometries > 5000:
            ...
            if added_geometries <= 0:
                try:
                    doc.removeObject(sketch_obj.Name)
                except Exception:
                    pass
                continue

            try:
                view_obj = getattr(sketch_obj, "ViewObject", None)
                if view_obj:
                    if is_issue:
                        view_obj.LineColor = (1.0, 0.1, 0.1)
                    else:
                        view_obj.LineColor = (0.0, 0.0, 0.0)
                    view_obj.LineWidth = 3.0
            except Exception:
                pass
            ...
            if is_issue:
                ...
                if issue_group:
                    try:
                        issue_group.addObject(sketch_obj)
                    except Exception:
                        pass
            else:
                created_machinable += 1
                if machinable_group:
                    try:
                        machinable_group.addObject(sketch_obj)
                    except Exception:
                        pass
        ...
        if issue_group and created_issue > 0:
            try:
                Gui.Selection.clearSelection()
                Gui.Selection.addSelection(doc.Name, issue_group.Name)
            except Exception:
                pass
        elif machinable_group and created_machinable > 0:
            try:
                Gui.Selection.clearSelection()
                Gui.Selection.addSelection(doc.Name, machinable_group.Name)
            except Exception:
                pass

        if (created_issue + created_machinable) > 0:
            print(
                f"Exported sketches: {created_issue} issue + {created_machinable} machinable "
                f"(folders: 'SVG Machinability Issues' and 'SVG Machinable')."
            )
        return issue_group

    before_names = {obj.Name for obj in (getattr(doc, "Objects", []) or [])}

    try:
        importSVG.insert(svg_path, doc.Name)
    except Exception as exc:
        print(f"SVG import failed: {exc}")
        return
    ...
    imported_objs = [obj for obj in (getattr(doc, "Objects", []) or []) if getattr(obj, "Name", "") not in before_names]
    if not imported_objs:
        print("SVG import produced no objects.")
        return

    def _is_non_cut_import_obj(obj):
        ...
        if any(token in text for token in ignore_tokens):
            return True

        return False

    filtered_imported_objs = []
    ignored_imported_objs = []
    for imported_obj in imported_objs:
        if _is_non_cut_import_obj(imported_obj):
            ignored_imported_objs.append(imported_obj)
        else:
            filtered_imported_objs.append(imported_obj)

    if ignored_imported_objs:
        print(f"Ignored {len(ignored_imported_objs)} obvious non-cut imported object(s) (guides/name/text-like).")
    imported_objs = filtered_imported_objs
    if not imported_objs:
        print("All imported objects were filtered as non-cut candidates; nothing to process.")
        return

    depth_mm = float(pad_depth_inch) * 25.4
    radius_mm = float(min_bit_radius_inch) * 25.4
    bit_diameter_mm = float(radius_mm) * 2.0

    def _faces_from_imported(obj):
        shape = getattr(obj, "Shape", None)
        if not shape:
            return []
        try:
        ...
        for wire in wires:
            try:
                if not wire or wire.isNull() or (not wire.isClosed()):
                    continue
                face = Part.Face(wire)
                if face and (not face.isNull()) and face.isValid():
                    built.append(face)
            except Exception:
                continue
        return built
    ...
    all_faces = []
    for imported in imported_objs:
        all_faces.extend(_faces_from_imported(imported))

    # Remove original imported path clutter once face data is captured.
    for imported in imported_objs:
        try:
            doc.removeObject(imported.Name)
        except Exception:
            try:
                if getattr(imported, "ViewObject", None):
                    imported.ViewObject.Visibility = False
            except Exception:
                pass

    if not all_faces:
        print("No valid closed faces found in imported SVG objects.")
        return

    prepared_faces = []
    prepad_offending = []
    auto_removed_corner_steps = 0
    auto_fixed_face_count = 0
    for idx, face in enumerate(all_faces, start=1):
        try:
            source_face = face.removeSplitter()
        except Exception:
            source_face = face

        label = f"s{idx}"
        corner_issues = _corner_interference_metrics(source_face, bit_diameter_mm, min_turn_deg=8.0)
        if corner_issues and attempt_tiny_feature_fix:
            cleaned_face, removed_steps = _remove_corner_interference_steps(
                source_face,
                bit_diameter_mm,
                min_turn_deg=8.0,
            )
            if int(removed_steps) > 0:
                source_face = cleaned_face
                auto_removed_corner_steps += int(removed_steps)
                corner_issues = _corner_interference_metrics(source_face, bit_diameter_mm, min_turn_deg=8.0)
                if not corner_issues:
                    auto_fixed_face_count += 1
        if corner_issues:
            try:
                smallest_step = min(float(item[0]) for item in corner_issues)
                prepad_offending.append((label, None, len(corner_issues), smallest_step))
            except Exception:
                prepad_offending.append((label, None, len(corner_issues), 0.0))

        prepared_faces.append((idx, source_face))

    if auto_removed_corner_steps > 0:
        print(
            f"CAM-style auto-fix removed {auto_removed_corner_steps} bit-limited corner step(s) "
            f"across {auto_fixed_face_count} face(s)."
        )

    if prepad_offending:
        print(
            f"Continuing with {len(prepad_offending)} shape(s) containing bit-limited corner artifacts; "
            "workflow will proceed without stopping."
        )

    group_name = _next_name(_safe_name("SVGInlayWorkflow", "SVGInlayWorkflow"))
    workflow_group = doc.addObject("App::DocumentObjectGroup", group_name)
    workflow_group.Label = "SVG Inlay Workflow"

    created_solids = []
    cleanup_updates_total = 0
    offending_infos = []
    for idx, source_face in prepared_faces:

        try:
            solid = source_face.extrude(App.Vector(0, 0, -depth_mm))
        except Exception:
            continue
        ...
        solid_name = _next_name(_safe_name(f"svg_inlay_solid_{idx}", "svg_inlay_solid"))
        solid_obj = doc.addObject("Part::Feature", solid_name)
        solid_obj.Shape = cleaned_solid
        solid_obj.Label = f"s{idx}"
        created_solids.append(solid_obj)

        for label, _, tiny_count, tiny_min in prepad_offending:
            if label == str(solid_obj.Label):
                offending_infos.append((label, solid_obj, tiny_count, tiny_min))
                break

        try:
            workflow_group.addObject(solid_obj)
        except Exception:
            pass

    if not created_solids:
        print("SVG workflow failed: could not create solids from imported faces.")
        try:
            doc.removeObject(workflow_group.Name)
        except Exception:
            pass
        return
    ...
    fillet_targets = list(created_solids)

    try:
        Gui.Selection.clearSelection()
        for target_obj in fillet_targets:
            Gui.Selection.addSelection(doc.Name, target_obj.Name)
    except Exception:
        pass

    print(
        f"SVG workflow: imported {len(imported_objs)} object(s), built {len(created_solids)} padded solid(s), "
        f"cleaned {cleanup_updates_total} micro-step artifact(s)."
    )

    fillet_for_cnc(
        fillet_radius_inch=float(min_bit_radius_inch),
        final_solid_name=str(output_label),
        preserve_unfilleted=True,
        allow_smaller_radius_fallback=False,
...
def continue_svg_inlay_from_selection(
    min_bit_radius_inch=None,
    pad_depth_inch=None,
    final_solid_name=None,
    show_dialog=True,
):
    print("Continue Inlay From Selection (SVG workflow) has been removed from this workbench.")
    return
    try:
        workflow_prefs = App.ParamGet("User parameter:BaseApp/Preferences/Mod/ButlerCues/SVGInlayWorkflow")
        default_min_bit_radius_inch = max(0.0001, float(workflow_prefs.GetFloat("min_bit_radius_in", 0.014)))
    ...
    if show_dialog and QtGui is not None and Gui is not None:
        class _ContinueFromSelectionTaskPanel:
            def __init__(self):
                self.form = QtGui.QWidget()
                self.form.setWindowTitle("Continue Inlay From Selection")
                self._last_run_signature = None
                ...
                note = QtGui.QLabel("Select edited faces/wires/solids first, then continue to pad + Z-axis fillet.")
            ...
            def _run_continue(self, close_after=True):
                current_signature = self._signature()
                if close_after and self._last_run_signature == current_signature:
                    try:
                        Gui.Control.closeDialog()
                ...
                try:
                    continue_svg_inlay_from_selection(
                        min_bit_radius_inch=selected_radius,
                        pad_depth_inch=selected_pad,
                        final_solid_name=selected_name,
                        show_dialog=False,
            ...
            def clicked(self, button):
                if button == QtGui.QDialogButtonBox.Apply:
                    self._run_continue(close_after=False)
                    return True
                return False

            def accept(self):
                return self._run_continue(close_after=True)
        ...
        Gui.Control.showDialog(_ContinueFromSelectionTaskPanel())
        return
    ...
    if not selection:
        print("No selection. Select edited faces/wires/solids first.")
        return
    ...
    def _safe_name(text, fallback="continue_inlay"):
        raw = str(text or "").strip()
        cleaned = "".join(ch if ch.isalnum() else "_" for ch in raw).strip("_")
        return cleaned or fallback

    group_name = _next_name(_safe_name("ContinueInlay", "ContinueInlay"))
    continue_group = doc.addObject("App::DocumentObjectGroup", group_name)
    continue_group.Label = "Continue Inlay"
    ...
    for item in selection:
        ...
        if solids:
            for solid in solids:
                try:
                    if solid and (not solid.isNull()):
                        solid_name = _next_name(_safe_name(f"cont_solid_{item.Name}", "cont_solid"))
                        solid_obj = doc.addObject("Part::Feature", solid_name)
                        solid_obj.Shape = solid
                        solid_obj.Label = str(getattr(item, "Label", item.Name) or item.Name)
                        created_solids.append(solid_obj)
                        try:
                            continue_group.addObject(solid_obj)
                        except Exception:
                            pass
                except Exception:
                    continue
        ...
        if not faces:
            ...
            for wire in wires:
                try:
                    if wire and (not wire.isNull()) and wire.isClosed():
        ...
        for face in faces:
            try:
                if not face or face.isNull():
                    continue
                solid = face.extrude(App.Vector(0, 0, -depth_mm))
                if hasattr(solid, "removeSplitter"):
                    try:
                        solid = solid.removeSplitter()
                    except Exception:
    ...
    if not created_solids:
        print("Could not build solids from current selection.")
        try:
            doc.removeObject(continue_group.Name)
        except Exception:
            pass
        return
    ...
    try:
        Gui.Selection.clearSelection()
        for solid_obj in created_solids:
            Gui.Selection.addSelection(doc.Name, solid_obj.Name)
    except Exception:
        pass

    print(f"Continue workflow: prepared {len(created_solids)} solid(s) from selection.")

    fillet_for_cnc(
        fillet_radius_inch=float(min_bit_radius_inch),
        final_solid_name=str(output_label),
        preserve_unfilleted=False,
        allow_smaller_radius_fallback=False,
        require_full_coverage=True,
        show_dialog=False,
    )
...
def clean_selected_issue_wires(
    max_tiny_edge_inch=None,
    create_closed_faces=False,
    hide_sources=True,
    show_dialog=True,
):
    print("Clean Issue Wires has been removed from this workbench.")
    return
    ...
    cleanup_prefs = None
    default_max_tiny_edge_inch = 0.003
    default_create_closed_faces = False
    default_hide_sources = True
    try:
        cleanup_prefs = App.ParamGet("User parameter:BaseApp/Preferences/Mod/ButlerCues/WireCleanup")
        default_max_tiny_edge_inch = max(0.0001, float(cleanup_prefs.GetFloat("max_tiny_edge_in", 0.003)))
        default_create_closed_faces = bool(cleanup_prefs.GetBool("create_closed_faces", False))
        default_hide_sources = bool(cleanup_prefs.GetBool("hide_sources", True))
    except Exception:
        cleanup_prefs = None

    if max_tiny_edge_inch is None:
        max_tiny_edge_inch = default_max_tiny_edge_inch
    try:
        max_tiny_edge_inch = max(0.0001, float(max_tiny_edge_inch))
    except Exception:
        max_tiny_edge_inch = default_max_tiny_edge_inch

    try:
        create_closed_faces = bool(create_closed_faces)
    except Exception:
        create_closed_faces = default_create_closed_faces

    try:
        hide_sources = bool(hide_sources)
    except Exception:
        hide_sources = default_hide_sources

    if show_dialog and QtGui is not None and Gui is not None:
        class _CleanIssueWiresTaskPanel:
            def __init__(self):
                self.form = QtGui.QWidget()
                self.form.setWindowTitle("Clean Issue Wires")
                self._last_run_signature = None

                layout = QtGui.QFormLayout(self.form)

                self.max_tiny_spin = QtGui.QDoubleSpinBox()
                self.max_tiny_spin.setDecimals(4)
                self.max_tiny_spin.setRange(0.0001, 1.0000)
                self.max_tiny_spin.setSingleStep(0.001)
                self.max_tiny_spin.setValue(float(max_tiny_edge_inch))
                self.max_tiny_spin.setSuffix(" in")

                self.create_faces_check = QtGui.QCheckBox("Create face objects from closed cleaned wires")
                self.create_faces_check.setChecked(bool(create_closed_faces))

                self.hide_sources_check = QtGui.QCheckBox("Hide original selected objects")
                self.hide_sources_check.setChecked(bool(hide_sources))

                note = QtGui.QLabel("Select issue wires/faces first. This removes tiny segments and rejoins edges.")

                layout.addRow("Drop segments shorter than", self.max_tiny_spin)
                layout.addRow("Closed loops", self.create_faces_check)
                layout.addRow("Visibility", self.hide_sources_check)
                layout.addRow("", note)

                try:
                    self.max_tiny_spin.valueChanged.connect(self._mark_dirty)
                    self.create_faces_check.toggled.connect(self._mark_dirty)
                    self.hide_sources_check.toggled.connect(self._mark_dirty)
                except Exception:
                    pass
            ...
            def _signature(self):
                return (
                    round(float(self.max_tiny_spin.value()), 6),
                    bool(self.create_faces_check.isChecked()),
                    bool(self.hide_sources_check.isChecked()),
                )

            def _run_cleanup(self, close_after=True):
                current_signature = self._signature()
                if close_after and self._last_run_signature == current_signature:
                    try:
                        Gui.Control.closeDialog()
                ...
                selected_max_tiny = max(0.0001, float(self.max_tiny_spin.value()))
                selected_create_faces = bool(self.create_faces_check.isChecked())
                selected_hide_sources = bool(self.hide_sources_check.isChecked())

                try:
                    if cleanup_prefs is not None:
                        cleanup_prefs.SetFloat("max_tiny_edge_in", float(selected_max_tiny))
                        cleanup_prefs.SetBool("create_closed_faces", bool(selected_create_faces))
                        cleanup_prefs.SetBool("hide_sources", bool(selected_hide_sources))
                except Exception:
                    pass

                try:
                    clean_selected_issue_wires(
                        max_tiny_edge_inch=selected_max_tiny,
                        create_closed_faces=selected_create_faces,
                        hide_sources=selected_hide_sources,
                        show_dialog=False,
                    )
                    self._last_run_signature = current_signature
                finally:
            ...
            def clicked(self, button):
                if button == QtGui.QDialogButtonBox.Apply:
                    self._run_cleanup(close_after=False)
                    return True
                return False

            def accept(self):
                return self._run_cleanup(close_after=True)
        ...
        Gui.Control.showDialog(_CleanIssueWiresTaskPanel())
        return

    selection = Gui.Selection.getSelection() if Gui else []
    if not selection:
        print("No selection. Select issue wires/faces first.")
        return

    threshold_mm = float(max_tiny_edge_inch) * 25.4
    ...
    def _safe_name(text, fallback="CleanWires"):
        raw = str(text or "").strip()
        cleaned = "".join(ch if ch.isalnum() else "_" for ch in raw).strip("_")
        return cleaned or fallback

    group_name = _next_name(_safe_name("CleanedIssueWires", "CleanedIssueWires"))
    cleaned_group = doc.addObject("App::DocumentObjectGroup", group_name)
    cleaned_group.Label = "Cleaned Issue Wires"

    created = []
    removed_edges_total = 0
    fallback_wires_total = 0
    preserved_wires_total = 0
    ...
    for obj in selection:
        ...
        try:
            source_wires = list(getattr(shape, "Wires", []) or [])
        except Exception:
            source_wires = []
        ...
        if not source_wires:
            ...
            if edges:
                try:
                    grouped = Part.sortEdges(edges)
                except Exception:
                    grouped = [edges]
                for edge_group in grouped:
                    try:
                        source_wires.append(Part.Wire(edge_group))
                    except Exception:
                        continue
        ...
        for source_wire in source_wires:
            try:
                if not source_wire or source_wire.isNull():
                    continue
            except Exception:
                continue

            try:
                wire_edges = list(getattr(source_wire, "Edges", []) or [])
            except Exception:
                wire_edges = []
            if not wire_edges:
                continue
            ...
            try:
                original_len = float(getattr(source_wire, "Length", 0.0) or 0.0)
            except Exception:
                original_len = 0.0
            try:
                original_closed = bool(source_wire.isClosed())
            except Exception:
            ...
            for edge in wire_edges:
                try:
                    ...
                    edge_len = 0.0
                if edge_len > 0.0 and edge_len < threshold_mm:
                    removed_local += 1
                    continue
                kept_edges.append(edge)

            removed_local_total += removed_local

            if removed_local == 0:
                output_shapes.append(source_wire)
                preserved_wires_total += 1
                continue

            if not kept_edges:
                output_shapes.append(source_wire)
                fallback_wires_total += 1
                continue

            try:
                grouped_clean = Part.sortEdges(kept_edges)
            except Exception:
                grouped_clean = [kept_edges]

            candidate_wires = []
            candidate_total_len = 0.0
            closed_mismatch = False
            for edge_group in grouped_clean:
                try:
                    candidate = Part.Wire(edge_group)
                except Exception:
                    continue
                try:
                    if original_closed and (not candidate.isClosed()):
                        closed_mismatch = True
                    candidate_total_len += float(getattr(candidate, "Length", 0.0) or 0.0)
                except Exception:
                    pass
                candidate_wires.append(candidate)

            length_ratio = (candidate_total_len / original_len) if original_len > 1e-9 else 1.0
            destructive = closed_mismatch or (length_ratio < 0.80) or (not candidate_wires)

            if destructive:
                output_shapes.append(source_wire)
                fallback_wires_total += 1
            else:
                output_shapes.extend(candidate_wires)

        removed_edges_total += removed_local_total

        for gidx, out_wire in enumerate(output_shapes, start=1):
            out_shape = out_wire
            out_label_suffix = "wire"
            if create_closed_faces:
                try:
                    if out_wire.isClosed():
                        face = Part.Face(out_wire)
                        if face and (not face.isNull()) and face.isValid():
                            out_shape = face
                            out_label_suffix = "face"
                except Exception:
                    pass

            base_label = str(getattr(obj, "Label", getattr(obj, "Name", "issue")) or "issue")
            out_name = _next_name(_safe_name(f"{base_label}_{out_label_suffix}", "clean_issue"))
            out_obj = doc.addObject("Part::Feature", out_name)
            out_obj.Shape = out_shape
            out_obj.Label = f"{base_label}_{out_label_suffix}_{gidx}"

            try:
                view_obj = getattr(out_obj, "ViewObject", None)
                if view_obj:
                    view_obj.LineColor = (0.1, 0.8, 0.1)
                    view_obj.LineWidth = 2.0
            except Exception:
                pass

            created.append(out_obj)
            try:
                cleaned_group.addObject(out_obj)
            except Exception:
                pass
    ...
    if created:
        try:
            ...
            for created_obj in created:
                Gui.Selection.addSelection(doc.Name, created_obj.Name)
        except Exception:
            pass
        print(
            f"Wire cleanup created {len(created)} object(s); removed {removed_edges_total} tiny segment(s) "
            f"(<={max_tiny_edge_inch:.4f} in)."
        )
        if fallback_wires_total > 0:
            print(f"Fallback preserved {fallback_wires_total} wire(s) to avoid destructive cleanup.")
        if preserved_wires_total > 0:
            print(f"Kept {preserved_wires_total} clean wire(s) unchanged.")
    else:
        try:
            doc.removeObject(cleaned_group.Name)
        except Exception:
            pass
        print("Wire cleanup did not produce any output objects.")


def split_non_touching_solids(
    target=None,
    hide_source=None,
    recurse_groups=None,
    shell_fallback=None,
    select_created=None,
    cleanup_xy_microsteps=None,
    microstep_max_inch=None,
    show_dialog=True,
):
    print("Split Non-Touching Solids has been removed from this workbench.")
    return
    ...
    default_recurse_groups = True
    default_shell_fallback = True
    default_select_created = True
    default_cleanup_xy_microsteps = True
    default_microstep_max_inch = 0.028
    try:
        split_prefs = App.ParamGet("User parameter:BaseApp/Preferences/Mod/ButlerCues/SplitSolids")
        default_hide_source = bool(split_prefs.GetBool("hide_source", True))
        default_recurse_groups = bool(split_prefs.GetBool("recurse_groups", True))
        default_shell_fallback = bool(split_prefs.GetBool("shell_fallback", True))
        default_select_created = bool(split_prefs.GetBool("select_created", True))
        default_cleanup_xy_microsteps = bool(split_prefs.GetBool("cleanup_xy_microsteps", True))
        default_microstep_max_inch = max(0.0001, float(split_prefs.GetFloat("microstep_max_inch", 0.028)))
    except Exception:
        split_prefs = None

    if hide_source is None:
        hide_source = default_hide_source
    try:
        hide_source = bool(hide_source)
    except Exception:
        hide_source = True

    if recurse_groups is None:
        recurse_groups = default_recurse_groups
    try:
        recurse_groups = bool(recurse_groups)
    except Exception:
        recurse_groups = True

    if shell_fallback is None:
        shell_fallback = default_shell_fallback
    try:
        shell_fallback = bool(shell_fallback)
    except Exception:
        shell_fallback = True

    if select_created is None:
        select_created = default_select_created
    try:
        select_created = bool(select_created)
    except Exception:
        select_created = True

    if cleanup_xy_microsteps is None:
        cleanup_xy_microsteps = default_cleanup_xy_microsteps
    try:
        cleanup_xy_microsteps = bool(cleanup_xy_microsteps)
    except Exception:
        cleanup_xy_microsteps = True

    if microstep_max_inch is None:
        microstep_max_inch = default_microstep_max_inch
    try:
        microstep_max_inch = max(0.0001, float(microstep_max_inch))
    except Exception:
        microstep_max_inch = default_microstep_max_inch

    microstep_max_mm = float(microstep_max_inch) * 25.4

    if show_dialog and QtGui is not None and Gui is not None:
        class _SplitSolidsTaskPanel:
            def __init__(self):
                self.form = QtGui.QWidget()
                self.form.setWindowTitle("Split Non-Touching Solids")
                self._last_run_signature = None

                layout = QtGui.QFormLayout(self.form)

                self.hide_source_check = QtGui.QCheckBox("Hide source object(s) after split")
                self.hide_source_check.setChecked(bool(hide_source))

                self.recurse_groups_check = QtGui.QCheckBox("Include members of selected groups")
                self.recurse_groups_check.setChecked(bool(recurse_groups))

                self.shell_fallback_check = QtGui.QCheckBox("Try shell split fallback for connected solids")
                self.shell_fallback_check.setChecked(bool(shell_fallback))

                self.select_created_check = QtGui.QCheckBox("Select newly created split solids")
                self.select_created_check.setChecked(bool(select_created))

                self.cleanup_xy_check = QtGui.QCheckBox("Cleanup sub-bit XY micro-steps before output")
                self.cleanup_xy_check.setChecked(bool(cleanup_xy_microsteps))

                self.microstep_size_spin = QtGui.QDoubleSpinBox()
                self.microstep_size_spin.setDecimals(4)
                self.microstep_size_spin.setRange(0.0001, 1.0000)
                self.microstep_size_spin.setSingleStep(0.001)
                self.microstep_size_spin.setValue(float(microstep_max_inch))
                self.microstep_size_spin.setSuffix(" in")

                note = QtGui.QLabel("Use Apply to run multiple times with different options.")

                layout.addRow("Visibility", self.hide_source_check)
                layout.addRow("Selection", self.recurse_groups_check)
                layout.addRow("Fallback", self.shell_fallback_check)
                layout.addRow("Post action", self.select_created_check)
                layout.addRow("Geometry cleanup", self.cleanup_xy_check)
                layout.addRow("Micro-step max", self.microstep_size_spin)
                layout.addRow("", note)

                try:
                    self.hide_source_check.toggled.connect(self._mark_dirty)
                    self.recurse_groups_check.toggled.connect(self._mark_dirty)
                    self.shell_fallback_check.toggled.connect(self._mark_dirty)
                    self.select_created_check.toggled.connect(self._mark_dirty)
                    self.cleanup_xy_check.toggled.connect(self._mark_dirty)
                    self.microstep_size_spin.valueChanged.connect(self._mark_dirty)
                except Exception:
                    pass
            ...
            def _signature(self):
                return (
                    bool(self.hide_source_check.isChecked()),
                    bool(self.recurse_groups_check.isChecked()),
                    bool(self.shell_fallback_check.isChecked()),
                    bool(self.select_created_check.isChecked()),
                    bool(self.cleanup_xy_check.isChecked()),
                    round(float(self.microstep_size_spin.value()), 6),
                )

            def _run_split(self, close_after=True):
                current_signature = self._signature()
                if close_after and self._last_run_signature == current_signature:
                    try:
                        Gui.Control.closeDialog()
                ...
                selected_hide_source = bool(self.hide_source_check.isChecked())
                selected_recurse_groups = bool(self.recurse_groups_check.isChecked())
                selected_shell_fallback = bool(self.shell_fallback_check.isChecked())
                selected_select_created = bool(self.select_created_check.isChecked())
                selected_cleanup_xy = bool(self.cleanup_xy_check.isChecked())
                selected_microstep_max = max(0.0001, float(self.microstep_size_spin.value()))

                try:
                    if split_prefs is not None:
                        split_prefs.SetBool("hide_source", bool(selected_hide_source))
                        split_prefs.SetBool("recurse_groups", bool(selected_recurse_groups))
                        split_prefs.SetBool("shell_fallback", bool(selected_shell_fallback))
                        split_prefs.SetBool("select_created", bool(selected_select_created))
                        split_prefs.SetBool("cleanup_xy_microsteps", bool(selected_cleanup_xy))
                        split_prefs.SetFloat("microstep_max_inch", float(selected_microstep_max))
                except Exception:
                    pass

                try:
                    split_non_touching_solids(
                        target=target,
                        hide_source=selected_hide_source,
                        recurse_groups=selected_recurse_groups,
                        shell_fallback=selected_shell_fallback,
                        select_created=selected_select_created,
                        cleanup_xy_microsteps=selected_cleanup_xy,
                        microstep_max_inch=selected_microstep_max,
                        show_dialog=False,
                    )
                    self._last_run_signature = current_signature
                finally:
            ...
            def clicked(self, button):
                if button == QtGui.QDialogButtonBox.Apply:
                    self._run_split(close_after=False)
                    return True
                return False

            def accept(self):
                return self._run_split(close_after=True)
        ...
        Gui.Control.showDialog(_SplitSolidsTaskPanel())
        return
    ...
    def _safe_name(text, fallback="SplitSolid"):
        raw = str(text or "").strip()
        cleaned = "".join(ch if ch.isalnum() else "_" for ch in raw).strip("_")
        return cleaned or fallback

    def _gather_targets(seed_objects):
        gathered = []
        seen = set()
        ...
        for obj in list(seed_objects or []):
            _add(obj)
            if recurse_groups:
                _walk_group(obj)
    ...
    def _shape_key(shape_obj):
        try:
            bb = shape_obj.BoundBox
            return (
                round(float(getattr(shape_obj, "Volume", 0.0) or 0.0), 6),
                round(float(bb.XMin), 5),
                round(float(bb.YMin), 5),
                round(float(bb.ZMin), 5),
                round(float(bb.XMax), 5),
                round(float(bb.YMax), 5),
                round(float(bb.ZMax), 5),
            )
        except Exception:
            return (id(shape_obj),)

    def _extract_split_candidates(shape_obj):
        collected = []
        seen = set()
        reasons = set()

        def _add_candidate(candidate, reason):
            if not candidate:
                return
            try:
                if candidate.isNull():
            ...
            if key in seen:
                return
            seen.add(key)
            collected.append((candidate, reason))
            reasons.add(reason)

        def _walk(current_shape):
            ...
            try:
                current_shape = current_shape.removeSplitter()
            except Exception:
                pass

            try:
                solids = list(getattr(current_shape, "Solids", []) or [])
            except Exception:
                solids = []

            if len(solids) > 1:
                for solid in solids:
                    _add_candidate(solid, "multi-solid")
                return

            if len(solids) == 1 and shell_fallback:
                try:
                    shells = list(getattr(solids[0], "Shells", []) or [])
                except Exception:
                    shells = []
                ...
                if len(shells) > 1:
                    for shell in shells:
                        try:
                            candidate = Part.Solid(shell)
                        except Exception:
                            continue
                        _add_candidate(candidate, "multi-shell-solid")
                    return

            nested = []
            try:
                nested.extend(list(getattr(current_shape, "Compounds", []) or []))
            except Exception:
                pass
            try:
                nested.extend(list(getattr(current_shape, "CompSolids", []) or []))
            except Exception:
                pass
            ...
            if len(solids) == 1:
                _add_candidate(solids[0], "single-solid")
        ...
        output_solids = [shape for shape, _ in collected]
        output_reasons = sorted(set(reason for _, reason in collected if reason != "single-solid"))
        if output_reasons:
            return output_solids, "+".join(output_reasons)
        return output_solids, "recursive"
    ...
    if target is not None:
        selected = [doc.getObject(target)] if isinstance(target, str) else [target]
    else:
        selected = Gui.Selection.getSelection() if Gui else []

    selected = _gather_targets([obj for obj in (selected or []) if obj])
    if not selected:
        print("No object selected. Select a solid/compound object first.")
        return
    ...
    for obj in selected:
        shape = getattr(obj, "Shape", None)
        if not shape:
            print(f"Skipping '{getattr(obj, 'Label', obj.Name)}': no shape.")
            continue
        try:
            if shape.isNull():
                print(f"Skipping '{getattr(obj, 'Label', obj.Name)}': null shape.")
                continue
        except Exception:
            pass

        solids, split_mode = _extract_split_candidates(shape)
        ...
        if len(solids) <= 1:
            try:
                ...
                shell_count = 0
            print(
                f"'{getattr(obj, 'Label', obj.Name)}' appears connected "
                f"(solids={len(list(getattr(shape, 'Solids', []) or []))}, shells={shell_count}); nothing to split."
            )
            continue

        split_sources += 1
        group_name = _next_name(_safe_name(f"{getattr(obj, 'Name', 'Object')}_split", "SplitSolids"))
        group = doc.addObject("App::DocumentObjectGroup", group_name)
        mode_suffix = " (shell split)" if split_mode == "multi-shell-solid" else ""
        group.Label = f"{getattr(obj, 'Label', obj.Name)} split solids{mode_suffix}"
        ...
        for idx, solid in enumerate(solids, start=1):
            ...
            cleaned_solid = solid
            cleaned_updates = 0
            if cleanup_xy_microsteps:
                try:
                    cleaned_solid, cleaned_updates = _cleanup_micro_xy_steps(solid, microstep_max_mm)
                except Exception:
                    cleaned_solid = solid
                    cleaned_updates = 0

            feature_name = _next_name(_safe_name(f"{getattr(obj, 'Name', 'Solid')}_solid", "SplitSolid"))
            split_obj = doc.addObject("Part::Feature", feature_name)
            split_obj.Shape = cleaned_solid
            split_obj.Label = f"s{idx}"
            created_objects.append(split_obj)
            try:
                group.addObject(split_obj)
            except Exception:
                pass
            created_total += 1
            if cleanup_xy_microsteps and cleaned_updates > 0:
                try:
                    print(
                        f"Cleanup for '{split_obj.Label}' removed/softened {int(cleaned_updates)} sub-bit XY edge artifact(s) "
                        f"(<= {microstep_max_inch:.4f} in)."
                    )
                except Exception:
                    pass
    ...
    if select_created and created_objects and Gui:
        try:
            Gui.Selection.clearSelection()
            for created_obj in created_objects:
                if created_obj and getattr(created_obj, "Name", None):
                    Gui.Selection.addSelection(doc.Name, created_obj.Name)
        except Exception:
            pass

    if created_total > 0:
        print(f"Created {created_total} split solid object(s) from {split_sources} source object(s).")
    else:
        print("No multi-solid objects found to split.")
    ...
    def _create_profile_ops_for_inlay(
        ...
        for selected_tc in selected_tcs:
            ...
            for outer_idx, outer_edges in enumerate(outer_edge_groups, start=1):
                ...
                inner_suffix = "_Inside" if len(outer_edge_groups) == 1 else f"_Outer{outer_idx}_Inside"
                op_specs.append(("Outside", outer_edges, outer_suffix))
                op_specs.append(("Inside", outer_edges, inner_suffix))
